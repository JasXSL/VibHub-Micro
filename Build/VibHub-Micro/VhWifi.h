...
const char JS_SHARED[] PROGMEM = "\n"
	...
	"QSA(\"div.msg, br\").forEach(e => e.remove());"
	// HOME
	"if(path==='/'){\n"
		...
		"let tx=D.createElement('h2');"
		"tx.innerText='LED Idle Color:';"
		"F.parentNode.insertBefore(tx,cdb.nextSibling);"
		"let clr=D.createElement('input');"
		"clr.type='color';"
		"clr.id='ledColor';"
		"F.parentNode.insertBefore(clr,tx.nextSibling);"
		...
		"let FE=(t,d,o)=>{"
			"fetch('/ajax?t='+t+'&d='+d).then(res=>res.text()).then(id=>{"
				"if(t.startsWith('id')){"
					"window.DEVID=id;"
					"uDID();"
				"}"
		...
		"};"
		"cdb.onsubmit=()=>{"
			"FE('ids', 'Secure ID Updated!');"
			"return false;"
		"};"
		"clr.onchange=()=>{"
			"FE('clr',parseInt('0x'+clr.value.slice(1)),'Color Updated!');"
		"};"
	...
	"if(path==='/wifi'||path==='/0wifi'){\n"
		...
		"let cDiv = QS(\"button[name=refresh]\");"
		"if(cDiv)"
			"cDiv.remove();\n"
